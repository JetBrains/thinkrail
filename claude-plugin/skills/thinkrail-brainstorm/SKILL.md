...
After that, you independently determine how the existing project specifications (in thinkrail style) should be amended to accurately reflect the project state once the feature is implemented.
The outcome of this step is a diff file describing the required changes to the thinkrail-style specifications.
...
Output files must be placed in `{{TR_DIR}}/<NAME>/` (see below for details).
...
1. ***Review the project context*** — examine the project’s thinkrail specification files and recent commits.
2. ***Create folder for design documents*** - create folder `{{TR_DIR}}/<NAME>/` and `git commit` where `<NAME>` is ticket_name (if any), current session name (if nice), or well-formulated short name for conversation (come up with the on yourself). Note `<NAME>` has to be unique and folder should not exist before, otherwise revise the `<NAME>`.
...
5. ***Write product design*** document - save to `{{TR_DIR}}/<NAME>/product-design.md` and `git commit`.
...
7. ***Propose*** three to five ***technical approaches***, each with a detailed description, a comparison, clear advantages and disadvantages, a thinkrail-style visualisation, and a well-reasoned recommendation.
...
9. ***Write technical design document*** — save to `{{TR_DIR}}/<NAME>/design-doc.md` and `git commit`.
...
11. ***Create a specification diff patch*** — based on the design documents, produce a diff patch for the existing thinkrail-style project specifications. These specifications must always reflect the current, up-to-date state of the project. The diff patch should update them to incorporate the user’s design changes. Present the changes to the user one specification file at a time and obtain explicit approval for each.
12. ***Write diff patch*** - save to `{{TR_DIR}}/<NAME>/spec-diff.patch` and `git commit`.
...
* First, review the current state of the project: examine the thinkrail specifications, code, and any other relevant files, particularly where specifications may be  missing.
...
* Use thinkrail-style visualisation wherever appropriate.

### Proposing solutions and approaches, Presenting the design

* Propose three to five approaches, each with a detailed description, a comparison, clear advantages and disadvantages, a thinkrail-style visualisation, and a well-reasoned recommendation.
...
* You MUST encourage the user to define — i.e. create thinkrail-style specifications for — existing parts of the project, or at least those on which the current work depends.
