...
- **domain.ts** — app entities: `Project` (git repo + unique `slug` + optional **`closed: true`** — the
  ...
  **`SUBAGENT_SETTINGS_PROTOCOL_VERSION`** pins the global/workspace controls to their v57 wire
  ...
  terminalReplayKb, composerGrowthLimit, chatLineWidth, fileLineWidth, chatLineWidthBounded,
  fileLineWidthBounded, customLayoutPresets, reviewModel?, reviewEffort?, reviewAutoFix, subagentsEnabled }`
  — an extensible bag; the line-width fields join the wire at protocol v59. `themeMode` defaults to
  `"fixed"` and no pair, preserving both legacy configs and the explicit Dark default; `subagentsEnabled`
  is the host-wide subagent default (`true` for current behavior), overridden only by
  ...
  **only** layout value synchronized by the host; current/default preset and group limits are web-local);
  ...
  fact on the wire: the installation id stays server-side by design, see `submodule-server-analytics`)
  carries it with the **`DEFAULT_CONFIG`** fallback (persisted host-side as `config.json`, delivered in
