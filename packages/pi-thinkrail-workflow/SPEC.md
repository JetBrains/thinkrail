...
position the old free-text append had. pi 0.87.1 still does not re-export that renderer from its package
root or `core/index.ts` (only its `BuildSystemPromptOptions`/`NormalizedBuildSystemPromptOptions` types
are root-exported), so the test reaches `dist/core/system-prompt.js` by resolved path; a future pi that moves it breaks the
