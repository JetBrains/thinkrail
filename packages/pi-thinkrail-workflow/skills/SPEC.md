...
flowchart LR
  ...
  createcard(["Create-project-from-scratch card"]) -->|/skill:starting-a-new-project in a draft project| pn
  sepcard(["Separate-task card"]) -->|/skill:kicking-off-a-workspace in a new workspace| kw[kicking-off-a-workspace]
...
| `starting-a-new-project` | worker — inception interview (empty repo → goal-and-requirements); a **hosted create-from-scratch** section (gated on the `finalize_project` tool) opens with a fixed lightweight 3-question brief (freeform, no widget), then drives **contextual** `ask_user_question` follow-ups from the accumulated answers, asks the name last, finalizes the draft project, and (hosted only) closes with a concise confirmation + an **`offer_next_steps`** two-card branch (separate task vs Default workspace) | `setting-up-a-project` + narrow self-trigger + the app's **Create-project-from-scratch** card seeding `/skill:starting-a-new-project` into a fresh draft project | active; route-in observed by the routing suite; the interview + hosted-entry finalize/offer unverified by use (rule 14 suspended) |
| `kicking-off-a-workspace` | worker — warm hand-off into a freshly created **isolated workspace**: one contextual line, read the committed brief, offer a few first tasks via `ask_user_question`, then route the pick through brainstorming | **outside the routing table** — self-trigger only via the app seeding `/skill:kicking-off-a-workspace` when the post-creation "Start a separate task" card opens a new workspace (rule 4) | active; unverified by use (rule 14 suspended) |
