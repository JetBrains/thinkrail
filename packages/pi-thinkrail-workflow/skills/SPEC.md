...
| `starting-a-new-project` | worker — inception interview (empty repo → goal-and-requirements); a **hosted create-from-scratch** section (gated on the `finalize_project` tool) opens by asking what to build, asks the name last, and finalizes the draft project | `setting-up-a-project` + narrow self-trigger + the app's **Create-project-from-scratch** card seeding `/skill:starting-a-new-project` into a fresh draft project | active; route-in observed by the routing suite; the interview + hosted-entry finalize unverified by use (rule 14 suspended) |
