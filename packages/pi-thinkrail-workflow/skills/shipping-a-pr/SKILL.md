...
- **Wait mode** applies to creating a PR, syncing its branch, any review fix that changes code, and an
  explicit request to watch checks, ship, or make the PR merge-ready. Done means every configured
...
- **Snapshot mode** applies to standalone screenshot, body, or comment-only maintenance that changes
  no code, and to one-time status requests, unless the user also asked for wait mode. Take one fresh
...
phase from this spine's summary. Carry the table's mode into that doc; an explicit watch, ship, or
merge-ready request overrides a snapshot default with wait mode, and creation carries wait mode
through a screenshot phase. A compound ask ("rebase, verify, and create a PR") is one flow: start at
...
Creating, syncing, code-affecting review fixes, and explicit watch/ship/merge-ready asks end through
`checks.md` in wait mode. Standalone metadata-only phases and one-time status requests end through
