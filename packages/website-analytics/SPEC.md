...
- When the supplied marketing-consent source reports granted, the package reads or creates a validated random first-party journey ID and registers it for subsequent events. Unknown/denied state never creates or reads journey storage. Withdrawal removes storage and in-memory registration. Events observed before grant are never retroactively enriched, and queued events lose enrichment if that grant is withdrawn before delivery.
- Typed captures are production-gated and may queue until PostHog initializes. Preview/disabled hosts never queue, persist journey state or touch vendor clients.
