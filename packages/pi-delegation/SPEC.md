...
stateDiagram-v2
    ...
    queued --> aborted: caller signal (released immediately)
...
| Abort | `signal` in `RunOptions` (tool abort, engine fail-fast) → child abort. An abort while still **queued** releases immediately (the caller never waits behind live runs; the eventual slot grant is handed straight back). Parent dispose → `disposeChildrenOf` cascade, which **marks every captured child disposed synchronously before awaiting any abort**: aborting the running child frees its semaphore slot, and an unmarked queued sibling would win that slot and issue a provider request mid-cascade only to be aborted moments later — marked first, it lands on the existing disposed-before-start guard instead (PR #302 review finding, lifecycle-pinned). A mid-turn parent abort kills only awaited (foreground) runs via their signals; detached runs survive turn aborts. |
