...
| Abort | `signal` in `RunOptions` (tool abort, engine fail-fast) and `ChildHandle.abort()` both cancel the active run through one run-scoped signal. The handle path must cancel a run that is still **queued**, not merely call `abort()` on its idle pi session: queued cancellation resolves immediately while the eventual slot grant is handed straight back, and no provider work starts (PR #302 review finding, regression-pinned). A running cancellation aborts the pi session. Parent dispose → `disposeChildrenOf` cascade, which **marks every captured child disposed synchronously before awaiting any abort**: aborting a running child frees its semaphore slot, and an unmarked queued sibling would win that slot and issue a provider request mid-cascade only to be aborted moments later — marked first, it lands on the existing disposed-before-start guard instead (earlier PR #302 review finding, lifecycle-pinned). A mid-turn parent abort kills only awaited (foreground) runs via their signals; detached runs survive turn aborts. |
