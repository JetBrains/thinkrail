...
Config validation normalizes the closed theme mode plus complete opaque system pair and the closed composer-growth preference, independently defaults malformed/missing chat and file line widths to 120 plus their pane-bound switches to `true`, and accepts only the current bounded `customLayoutPresets` catalog as synchronized layout data. Current/default preset ids, group limits, and chat message order are not config fields; retired config shapes are stripped rather than upgraded or preserved as extensions. Historical `layouts/` files remain untouched and inert.
