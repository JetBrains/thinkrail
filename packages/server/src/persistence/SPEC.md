...
Config validation normalizes the closed composer-growth and chat-message-order preferences and accepts only the current bounded `customLayoutPresets` catalog as synchronized layout data. Current/default preset ids and group limits are not config fields, and retired config shapes are ignored rather than upgraded. Historical `layouts/` files remain untouched and inert.
