...
- **Owns:** `dataDir()` (`THINKRAIL_DATA_DIR` for dev/e2e isolation, else `~/.thinkrail`);
  ...
  (`config.json`, fieldwise-normalized over `DEFAULT_CONFIG`—including the closed composer-growth and
  chat-message-order preferences plus both nested layout group limits—so a missing/invalid known value or
  key degrades cleanly, while unknown top-level extension fields survive known-field updates),
