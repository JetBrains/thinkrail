...
- **Public surface (barrel):** the lifecycle operations, publisher installer, action type, and test reset.
- **Allowed deps:** `persistence` (`dataDir` only); Node filesystem/path APIs.
...
- `host` calls `recordAcceptedMessage` only after `ackSend` accepts a non-control `prompt`, `steer`, or `follow_up`. The synchronous persist happens inside the replay-cached request execution, so request replay cannot double-count.
- Reaching `nextInvitationAt` claims at most one opaque client key and invokes the injected addressed publisher. While claimed, other clients do not receive the popup. Failed delivery or client disconnect releases the memory-only claim; restart also releases it while durable eligibility remains.
