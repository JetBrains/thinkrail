...
- **Owns:** `git(cwd, args)` (spawn git *sync*, capture trimmed stdout/stderr + ok; `opts.raw` keeps
  ...
  args)` (its async twin — off the event loop through `subprocess`' `runBounded`, same `raw` option, for
  network-bound ops like `fetch` **and** the request-path reads below, neither of which may block the host:
  it owns only the git-shaped part, the 55s budget and the
