...
- **Attention fan-out:** before accepting requests, `createServer` awaits the agent module's attention-ledger
  initialization barrier; failure aborts startup rather than admitting work under an ambiguous migration.
  It then installs the attention publisher and broadcasts each `SessionAttentionPayload` on
  `session.attention`, which every client subscribes beside the other session
  channels. `session.attentionList` supplies the authoritative all-workspace snapshot because pushes are not
  ...
  clients. `todoReview` passes `userVisible: false` for reviewer/reflector creation and pinned reattachment,
  so their persisted sessions produce neither attention nor live-running presentation. The host satisfies the shared `setSessionProjectResolver` from the workspace registry
...
- **Live-running fan-out:** `createServer` installs the agent module's running publisher and broadcasts each
  ...
  authoritative currently-streaming, user-visible top-level entries from the live agent registry; unlike
  attention it needs neither workspace records nor disk reconstruction. User chat creation keeps the default
  visible policy. Pushes are not replayed, so
