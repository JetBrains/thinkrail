...
- **Workspace model preference is host composition, not agent policy.** New-session precedence is:
  ...
  **Persistence is best-effort and never demotes a mutation that already succeeded** (the `track()` rule): the
  ...
  nothing. The plan-review subagent stays on the separate `reviewModel` / `reviewEffort` policy (else the
  user's global default, [[submodule-server-host-plan-review]]) and never inherits or updates workspace
  preference.
