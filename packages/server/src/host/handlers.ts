...
const handlers: Record<string, Handler> = {
	...
	// `scope` selects what is diffed (branch / working-tree / staged / one commit / one pinned commit;
	// omitted = branch). A scope naming a commit that no longer exists rejects — the panel resets its
	// scope on that rejection.
	"git.status": (params) => {
