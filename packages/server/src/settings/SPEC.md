...
- `subagentsEnabled` defaults to `true` when absent so old config preserves current behavior; a present non-boolean update is rejected before cache, persistence, or broadcast changes. `updateChecksEnabled` (the outbound-release-check switch the host tees into [[submodule-server-update]]) follows exactly that rule: **every boolean the host acts on is validated here**, because the merge's untyped remainder would otherwise persist and broadcast whatever a client sent. Settings owns only that global default; workspace override and effective-value resolution stay outside this module.
