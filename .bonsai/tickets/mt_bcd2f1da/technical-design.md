...
which **persists a draft to `.bonsai/sessions/{id}.json` and broadcasts
...
**+ New** mints the `bonsaiSid` client-side (UUID) and inserts a normal
...
Saving calls `agent/prepare` **passing the already-minted `bonsaiSid`** —
`Tracker.create_task` already accepts an optional `bonsai_sid`
...
```
 ...
 (frontend only)      (.bonsai/sessions)
  ...
  (no RPC,             agent/prepare(bonsaiSid)   (enqueues typed
...
2. **Stable identity:** the `bonsaiSid` is fixed from creation, so
...
| `frontend/src/store/draftAutosave.ts` | Store-level autosave controller (module-scoped, not a React hook). Holds `Map<bonsaiSid, { trailingTimer, maxWaitTimer }>`. `noteInput(bonsaiSid)` (re)arms a **750 ms trailing** timer and a **5 s max-wait** timer; `flush(bonsaiSid)` cancels timers and commits immediately; `cancel(bonsaiSid)` drops timers (on discard/end). Commit logic delegates to session-store actions. Survives component unmount; imported by `sessionStore` and `InputArea`. |
...
| `store/sessionStore.ts` | **`createNewSession`**: no longer calls `createDraft`/`agent/prepare`. First runs the **no-duplicate-blanks** guard (focus an existing `unsaved` + empty-input draft if one exists); else mints a `bonsaiSid` (`crypto.randomUUID()`) and inserts a `Session` with `status:"draft"`, `unsaved:true`, default name `"New session"`, locally-built default config. **New `ensureSaved(bonsaiSid)`**: if `unsaved`, calls `agent/prepare` **with the minted id** + held config + `draftInput` + derived name, then flips `unsaved:false` (idempotent / single-flight). **`noteDraftInput(bonsaiSid, text)`**: live-derives `session.name` when `!nameManuallySet`, then routes to `draftAutosave.noteInput` (only arms a save when text ≥ 5 non-ws **or** already saved). **`commitDraft(bonsaiSid)`** (called by the controller): `ensureSaved` on first save, else `agent/updateDraft({ draftInput, name })`. **`updateDraft`**: when `unsaved`, apply changes to the in-memory session **without** the RPC (config held locally). **`startDraft`/`sendMessage`**: `await ensureSaved` before the existing `agent/startDraft`. **`renameDraft(bonsaiSid, name)`**: sets `nameManuallySet:true`, updates name (local if unsaved, debounced `updateDraft` if saved). **`restoreSession`/`loadActiveSessions`**: seed `inputDraftStore` from the entry's `draftInput` and set `session.name` from persisted name. **`discard`/`endSession`**: `draftAutosave.cancel`; unsaved discard is purely local (no RPC). |
...
| `agent/tracker.py` | `create_task` already accepts `bonsai_sid`; add a `draft_input` param (or set `task.draft_input` after construction) so `prepare_task` can thread both. |
| `agent/service.py` | `prepare_task(...)`: accept `bonsai_sid: str \| None = None` and `draft_input: str \| None = None`, pass both to `create_task`. `update_draft(...)`: add `draft_input` (Ellipsis-sentinel) and persist it. `_save_task`: write `draftInput`. `list_all_sessions`: include `draftInput` in the draft-only fields (alongside `config`/`systemPrompt`/`sessionPrompt`). Disk-rehydration on startup reads `draftInput`. |
| `agent/persistence.py` | `save_session`: persist `draftInput`. `list_sessions`: include `draftInput` in the `status == "draft"` branch. |
| `rpc/methods/agents.py` | `prepare_agent`: read `bonsaiSid` (optional) and `draftInput` from params, forward to `prepare_task`. `update_draft`: read `draftInput` and forward. (Both handlers stay `**params`-based; no Pydantic request model exists today.) |
...
`commit` is wired by `sessionStore` to call `commitDraft(bonsaiSid)`.
...
//   (no-duplicate-blanks); else mint bonsaiSid + insert unsaved draft.

ensureSaved(bonsaiSid): Promise<void>;
//   if unsaved → agent/prepare({ bonsaiSid, config, skillId, specIds, filePaths,
//   name, draftInput }) then set unsaved=false. Single-flight; no-op if saved.

noteDraftInput(bonsaiSid, text): void;
//   live-derive name (when !nameManuallySet); arm autosave only when
//   nonWs(text) ≥ SAVE_THRESHOLD OR session already saved.

commitDraft(bonsaiSid): Promise<void>;
//   unsaved → ensureSaved; else agent/updateDraft({ draftInput, name }).

renameDraft(bonsaiSid, name): void;
//   nameManuallySet=true; local if unsaved, debounced updateDraft if saved.

updateDraft(bonsaiSid, changes): Promise<string>;  // UNCHANGED signature.
...
```json
{
  "bonsaiSid": "client-minted-uuid",   // NEW: reuse instead of server-mint
...
Response unchanged: `{ bonsaiSid, systemPrompt, sections, totalTokens }` (echoes
the supplied `bonsaiSid`).

`agent/updateDraft` request — one **optional, additive** field:

```json
{ "bonsaiSid": "…", "draftInput": "fix login flow", "name": "fix login flo…" }
```
...
async def prepare_task(self, spec_ids, config, skill_id=None, session_prompt=None,
    ...
    bonsai_sid: str | None = None, draft_input: str | None = None) -> AgentTask

async def update_draft(self, bonsai_sid, *, …, draft_input: str | None = ...) -> str
```

### Compatibility

All wire additions are optional. Omitting `bonsaiSid` preserves today's
server-mint behavior (used by `agent/run` and pre-configured prepares); omitting
`draftInput` leaves it `None`. No migration of existing `.bonsai/sessions/`
...
noteDraftInput → arms trailing(750ms) + maxWait(5s)
  ...
  trailing fires → commitDraft(id)
     → unsaved → ensureSaved(id)
          → agent/prepare({ bonsaiSid:id, config, skillId, specIds,
                            filePaths, name, draftInput:text })
          → backend persists .bonsai/sessions/{id}.json (status draft)
...
| **Concurrent save triggers** (threshold timer fires while Start is also pressed) | `ensureSaved` is **single-flight**: store the in-flight promise per `bonsaiSid` (same pattern as `_restoring`/`_subscribed`). The second caller `await`s the first; only one `agent/prepare` runs, so only one draft is ever created. |
...
- **`prepare_task` honors a caller-supplied `bonsai_sid`** — the persisted task
  and returned `bonsaiSid` equal the supplied id (no server re-mint).
...
- **`draft_input` is non-context** — assert the built system prompt does **not**
  ...
  `session_prompt`).
- **Backward-compat** — `prepare_task` with no `bonsai_sid`/`draft_input` behaves
...
- "no file on disk" assertions check `.bonsai/sessions/`; "no broadcast" is
...
1. **Empty abandon** — `+ New`, type nothing, switch/reload: no
   `.bonsai/sessions/{id}.json`; a second client's list gains no entry.
...
12. **Single draft** — concurrent threshold-fire + Start produce exactly one
    `agent/prepare` (single-flight `ensureSaved`); the minted `bonsaiSid` is
