...
Defines the backend storage model. Bonsai is a **single-user, localhost-only** developer tool (see `GOAL&REQUIREMENTS.md`); there is no concept of accounts, tokens, or authentication. Storage is split into two scopes:

- **App scope** — SQLite database at `~/.bonsai/bonsai.db` for app-wide state that outlives any single project (the known-projects registry, app settings).
- **Project scope** — `<project>/.bonsai/` directory for everything that belongs to a particular project (sessions, specs, tickets, plans, project-local settings).
...
- Compatible with the existing `aiosqlite` access pattern and `~/.bonsai/` data directory
- Keep per-project storage in `<project>/.bonsai/` unchanged

**Non-Goals:**
- Authentication or user identity — Bonsai is single-user; the OS user owns the data
...
graph TD
    ...
    subgraph Browser["Browser localStorage"]
        UI["bonsai-ui (panels, active tab)"]
        TH["bonsai-theme"]
        SO["bonsai-notification-sound"]
        MH["bonsai-message-history"]
        FT["bonsai-filetree-*"]
        LP["bonsai-last-project (cached)"]
    end
```

| Scope | Location | Contains |
|-------|----------|----------|
| **App** | `~/.bonsai/bonsai.db` (override via `$BONSAI_DATA_DIR`) | Known projects, app settings |
| **App (indexes)** | `~/.bonsai/indexes/<hash>/index.db` | Per-project spec index — generated cache, rebuildable |
| **Project** | `<project>/.bonsai/` | Sessions, specs, tickets, plans, project-local settings |
...
`~/.bonsai/bonsai.db` — overridable via `$BONSAI_DATA_DIR` environment variable or `data_dir` in `.env`. Created lazily on first backend startup. The `~/.bonsai/` directory is the **app** data directory, completely separate from any project's `.bonsai/` folder.
...
| Key | Purpose |
|-----|---------|
| `bonsai-ui` | Panel collapse, active tab |
| `bonsai-theme` | Theme preference |
| `bonsai-notification-sound` | Sound toggle |
| `bonsai-message-history` | Chat input history |
| `bonsai-filetree-*` | Filetree expand/collapse |
| `bonsai-last-project` | Path of the most recently opened project (cache) |

The `bonsai_token` key is **removed** from the frontend.
...
| Two tables only | `settings` + `projects` | Bonsai is single-user; that's all we need. Anything more is speculative. |
...
| `~/.bonsai/` data dir | Unchanged | Established convention; `$BONSAI_DATA_DIR` for overrides (containers, tests). |
| Project storage unchanged | `<project>/.bonsai/` | Per-project data stays where developers expect. |
