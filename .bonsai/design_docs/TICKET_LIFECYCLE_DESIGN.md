...
| `amend-specs`          | `ticket-amend-specs`           | mutations to `.bonsai/design_docs/*.md` recorded in `history.patch` |
...
The only automatic commit driven by the lifecycle fires on `amend-specs → implementation-plan`: `BoardService.on_status_change` runs `git add` + `git commit` on `.bonsai/design_docs` and the ticket's `history.patch`, with message `[ticket {id}] amend specs`. Failures are logged and swallowed (best-effort). All other artifact writes — `product-design.md`, `technical-design.md`, `implementation-plan.md` — are not auto-committed by the backend; the user commits manually when convenient.
...
The folder is created at `create_ticket` time alongside `ticket.json` so partial state is impossible. `list_tickets` walks `.bonsai/tickets/*/ticket.json` and silently skips subdirectories without a `ticket.json`, which lets folders survive crash-mid-write without polluting the board.

Legacy data is removed, never migrated. On every `list_tickets` call, `wipe_legacy_meta_tickets` checks for and recursively removes `.bonsai/meta-tickets/`. A pre-existing `spec-diff.patch` (the old filename for the amendment log) is renamed to `history.patch` on reconciliation. There is no compatibility shim.

`trash_ticket(ticket_id)` archives the entire `.bonsai/tickets/{id}/` folder as a single trash item; no per-artifact cascade.
...
| Method                | Payload |
|-----------------------|---------|
| `ui/artifactAdded`    | `{ bonsaiSid, artifact: SessionArtifact }` |
| `ui/artifactLabeled`  | `{ bonsaiSid, path, role?, label? }` |
| `ui/setPreviewFile`   | `{ bonsaiSid, path: string \| null, section? }` |
...
| `ticket-amend-specs`         | `product-design.md`, `technical-design.md`, specs via `spec_search` | mutations to `.bonsai/design_docs/*.md` (each appended to `history.patch`) | `technical-design → amend-specs → implementation-plan` |
...
Operates on `.bonsai/design_docs/*.md`, not on a per-ticket artifact:
...
3. Propose an amendment plan ordered general → specific (goals → architecture → modules → submodules → task specs), rendered as a `bonsai_visualize` summary box. Iterate via `AskUserQuestion` until approved.
...
6. `ClearPreviewFile`.
7. `AskUserQuestion` → `ChangeTicketStatus("implementation-plan")`, which triggers the single commit covering `.bonsai/design_docs` + the ticket's `history.patch`.
...
How the orchestrator dispatches each step depends on the *execution mode* chosen at launch — child Bonsai sessions, or SDK subagents in the orchestrator's own session. The full mode matrix and its data flow are described under [Implementation orchestration modes](#implementation-orchestration-modes).
...
Global setting `tickets.subagentFailurePolicy` in `.bonsai/settings.json`, default `"fail-fast"`:
...
**UX verification on `/Users/danya/projects/aiir/demo-board/`** — a real-browser smoke test driven via `chrome-devtools-mcp`, run against the demo project so the main Bonsai checkout stays clean. Iterate fixes against this checklist until every item passes without intervention:
...
10. Edit `.bonsai/settings.json` → `tickets.subagentFailurePolicy: "wait-all"`. Restart backend. Force a subagent failure (broken prompt or non-existent file in the step prompt). Verify the orchestrator waits for siblings before reporting.
