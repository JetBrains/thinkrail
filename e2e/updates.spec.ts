...
import { E2E_UPDATE_CONTROL } from "./fixtures/paths";

const BANNER = '[data-testid="update-banner"]';

test("release awareness: check → banner → dismiss → install → restart to finish", {
	tag: "@dev-seam",
}, async ({ page }) => {
	await openAppFresh(page);
	writeFileSync(E2E_UPDATE_CONTROL, "available");
	...
	// Leaves no banner behind: this lane's host keeps its staged state for every later spec.
	await page.getByTestId("update-banner-dismiss").click();
	await expect(page.locator(BANNER)).toHaveCount(0);
});
