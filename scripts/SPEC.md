...
- **Owns:** the multi-process development launcher; exact-version/catalog validation; the PI binary-seam canary; module dependency/import boundary validation; declared-spec-surface to TypeScript-barrel conformance; and spec-corpus readability + graph-validity linting.
...
`check:specs` lints the tracked spec corpus for readability and graph validity. Budgets: 3,000 words per spec file (visible per-spec override via frontmatter `spec-budget`, hard ceiling 4,500); 120 words per prose block — a paragraph, or a list item's own text with nested sub-items counted as their own blocks; at least one `##`+ heading per 500 words. Graph validity runs the `spec_validate` checks (dangling links, duplicate ids, parent cycles) plus the rule that durable specs never link `task-spec` ids; task-specs themselves are budget-exempt but graph-checked.

Counting is CommonMark-structural (frontmatter and fenced/indented code excluded; inline code and link text count, URLs don't), and scoping is `git ls-files` so a local tree and CI lint the same corpus. The check ships warn-only (reports, exits 0) until the corpus convergence flips it to `--enforce` in pre-commit and CI; filesystem-level fixtures pin the counting semantics, the override, the graph rules, and the runner's exit codes.
