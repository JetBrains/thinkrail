...
parent: architecture
references: [module-contracts, module-server, module-web, module-cli, module-desktop, module-ci-release, module-spec-graph]
...
- **Allowed deps:** Bun/Node, the TypeScript compiler API, and `pi-spec-graph/core` for the canonical spec/frontmatter model; read-only inspection of workspace manifests and source trees; the public package metadata and bundle outputs each check validates.
...
The TypeScript compiler resolves the barrel's effective export names, including type-only and default exports and transitive re-exports; an unresolved re-export is a violation rather than a silently incomplete surface. The declared and effective name sets must match in both directions. Filesystem-level tests exercise enrollment, resolution, and failure behavior through the same runner CI invokes.
