...
|---------|---------------|
| Models | `SuccessCriterion`, `PlanStep`, `Plan` — Pydantic models with camelCase serialization |
...
| `PlanStep` | number: `int`, title: `str`, status: `Literal["pending", "executing", "done", "failed"]`, skill: `str` (default `"default"`), depends_on: `list[int]`, input_spec_ids: `list[str]`, session_id: `str \| None`, success_criteria: `list[SuccessCriterion]` | One step in the execution plan |
| `Plan` | ticket_id: `str`, title: `str`, status: `Literal["draft", "ready", "executing", "done"]`, steps: `list[PlanStep]`, verification: `list[SuccessCriterion]` | Full plan document |
...
| `create_plan` | `(ticket_id: str, title: str, steps: list[PlanStep], verification?: list[SuccessCriterion]) -> Plan` | Create new plan with `draft` status and write to disk |
...
Plans are rendered as structured Markdown with three sections:
...
| Steps | Split by `### Step N: title` headings | Empty list |
