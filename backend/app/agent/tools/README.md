...
from app.agent.tools.visualization import vis_mcp_server, intercept_visualize
...
| Suffix matching for tool names | `INTERCEPTORS` keys are suffixes, not exact names | MCP tools may be prefixed by server name (e.g., `mcp__bonsai-vis__bonsai_visualize`). Suffix match handles both prefixed and unprefixed names. |
...
| Graduation to own package | Complex features move to `backend/app/<feature>/` | Prevents tools/ from becoming a dumping ground. If a tool needs models.py + service.py + multiple files, it's graduated (like `vis/` did). |
...
- **Tool specs:** [SuggestSession](SUGGEST_SESSION.md), [Visualization](VISUALIZATION.md), [UpdateProgress](PROGRESS.md)
