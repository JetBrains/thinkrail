...
from app.core.config import AppConfig, BONSAI_DIRNAME, IMPLEMENTATION_TASKS_DIR
...
class VisualizationService:
    ...
    def _parse_tasks(self) -> list[TaskEntry]:
        ...
        task_dir = self._root / BONSAI_DIRNAME / IMPLEMENTATION_TASKS_DIR
        if not task_dir.is_dir():
            return tasks
        pattern = str(task_dir / "**" / "*.md")
        for fpath in sorted(glob.glob(pattern, recursive=True)):
            rel = os.path.relpath(fpath, self._root)
            parts = rel.replace(os.path.join(BONSAI_DIRNAME, IMPLEMENTATION_TASKS_DIR) + os.sep, "").split(os.sep)
