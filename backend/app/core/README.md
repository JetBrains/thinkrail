...
| `ProjectSettings` | model_refresh_interval_hours, event_view, user_respond_timeout, user_respond_timeout_behavior, user_respond_retry_max_attempts, trash_retention_days, voice_revise_mode | User-configurable per-project settings (`.bonsai/settings.json`). Session-creation defaults (model / effort / permission_mode / max_turns) are *not* here — they're user-scoped, see `SessionDefaults` below. Timeout settings control what happens when user doesn't respond to an `AskUserQuestion` or `confirmAction` within the configured period. `trash_retention_days` (default `30`) controls auto-purge of trashed items; `0` disables auto-purge. `voice_revise_mode` (default `"off"`) controls what happens to a voice transcript after recording: `"off"` leaves the raw transcript alone; `"auto"` runs a one-shot server-side revise (see [VOICE_INPUT_DESIGN.md](../../../.bonsai/design_docs/VOICE_INPUT_DESIGN.md) — Revision 2); `"subsession"` starts a refinement subsession (v1 behavior). |
| `SessionDefaults` | model, permission_mode, effort, max_turns | User-scoped session-creation defaults — applied to every new draft across every project. Stored as a JSON value in `AppStore.settings` under the `session_defaults` key (see `app.core.session_defaults`). Cold-start values: `model=claude-opus-4-7`, `permission_mode="default"`, `effort=None` (renders as "auto" in the UI), `max_turns=50`. Read/written through the `appSettings/getSessionDefaults` and `appSettings/setSessionDefaults` RPCs. |
