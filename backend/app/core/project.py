"""Lazy auto-creation of ``.tr/`` meta-files and subdirectories.
...
The ``.tr/`` directory itself is never created eagerly — it appears
...
# ---------------------------------------------------------------------------
# Each callable receives the ``.tr/`` directory and returns the file
# content as a string.


def _default_settings(thinkrail_dir: Path) -> str:
...
def ensure_meta_file(thinkrail_dir: Path, rel_path: str) -> str:
    """Read ``.tr/{rel_path}`` if it exists; create it with defaults otherwise.

    Returns the file content (existing or freshly generated).
    Raises :class:`ValueError` if *rel_path* is not a known meta-file.
    """
    file_path = thinkrail_dir / rel_path
    ...
    content = factory(thinkrail_dir)
    write_text(file_path, content)
    return content


def ensure_meta_dir(thinkrail_dir: Path, name: str) -> Path:
    """Ensure ``.tr/{name}/`` exists.  Returns the directory path."""
    dir_path = thinkrail_dir / name
    ensure_dir(dir_path)
    return dir_path
