"""Tests for the thinkrail-amend MCP tool (ProposeChange)."""
...
async def _resolve_pending(tracker: Tracker, thinkrail_sid: str, response: dict) -> None:
    ...
    for req_id in list(tracker._futures.get(thinkrail_sid, {})):
        tracker.resolve_future(thinkrail_sid, req_id, response)
        break
...
async def test_propose_change_accept_applies_and_logs(tmp_path: Path) -> None:
    ...
    assert payload["filePath"] == ".tr/design_docs/X.md"
    # Tool result JSON
...
async def test_propose_change_discuss_no_apply(tmp_path: Path) -> None:
    ...
    # No log
    log_path = tmp_path / ".tr" / "tickets" / ticket.id / "history.patch"
