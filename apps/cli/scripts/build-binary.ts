...
const bundledExtensions: { specifier: string; skills: boolean }[] = [
	{ specifier: "pi-web-access/index.ts", skills: false },
	{ specifier: "pi-visualize/index.ts", skills: false },
	{ specifier: "pi-next-steps/index.ts", skills: false },
...
function generateBundledExtensions(): void {
	const resolved = bundledExtensions.map((extension) => ({
		...extension,
		path: serverRequire.resolve(extension.specifier),
	}));
	const factoryImports = resolved.map(
		(extension, i) => `import x${i} from ${JSON.stringify(extension.path)};`,
	);
	const factoryList = resolved.map((_, i) => `x${i}`).join(", ");
	const skillRoots = resolved
		.filter((extension) => extension.skills)
		.map((extension) => join(dirname(extension.path), "skills"));
