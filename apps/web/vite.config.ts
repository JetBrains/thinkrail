...
export default defineConfig({
	...
	server: {
		// The dev launcher (`bun run dev`) pre-picks a free port and pins it here via THINKRAIL_WEB_PORT
		...
		proxy: {
			// The dev launcher (`bun run dev`) sets THINKRAIL_PORT to the host's free port; match it.
			"/ws": {
