...
- **`src/themes` is the theme contract and catalog; `src/styles/tokens.css` is structural.** A bundled
  ...
  radii, motion, generic derived formulas and thin font aliases onto the generated typography tokens — no
  typography values and no named theme blocks.
...
- Token names that collide with Tailwind namespaces (`--font-mono`, `--font-accent`, `--radius-*`) are
  used as token arbitrary values (`rounded-[var(--radius-md)]`), not `@theme` mappings.
- **Typography comes from `styles/typography.json` via generated semantic classes — nothing else.**
  ...
  by `typography:check`) emits the primitive `--tr-*` custom properties plus one class per semantic
  style: `tr-brand-*`, `tr-title-*`, `tr-text-*`, `tr-code-*` and the shared `tr-prose`. A call site
  names exactly one of them and adds its own **colour**; `italic` and `leading-*` are the only Tailwind
  utilities that may override a semantic style (the classes are emitted in `@layer components`, so a
  utility wins the single property it names).
- **Handwritten typography utilities must not be introduced.** No new `@utility` rule may set
  ...
  `text-mono` / `text-base-mono` / `text-brand` / `text-eyebrow` utilities are gone; `tr-code-text`,
  `tr-code-inline`, `tr-brand-wordmark` / `tr-brand-hero` and `tr-text-eyebrow` replace them. The
  primitive `text-xs|sm|base|md|lg` mappings survive in `index.css` only as the escape hatch for
  third-party integration, never for ordinary components (`styles/typographyUsage.test.ts` enforces it).
- **A primitive font family may only be named for a documented third-party integration.** Monaco
  ...
  is exhaustive and lives in `styles/typographyUsage.test.ts`. Everywhere else a class is required. Note
  that the bare arbitrary value `font-[var(--font-mono)]` is ambiguous — Tailwind compiles it to an
  invalid `font-weight`, so it silently does nothing; the working form is
  `font-(family-name:--font-mono)` (`styles/fontClasses.test.ts` fails on the bare form).
...
- **The typography system — `typography.json` as the single source of truth, the primitives and the
  semantic styles generated from them, the 400/500/600 weight policy, code-only mono, the shared prose
  system, and how to add or change a style — is specced in
  [src/styles/TYPOGRAPHY.md](src/styles/TYPOGRAPHY.md)** (`web-typography`); check changes against it.
