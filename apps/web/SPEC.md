...
- **The typography system — `typography.json` as the single source of truth, the primitives and the
  semantic styles generated from them, the `<body>` base, the 400/500/600 weight policy, code-only mono,
  the two prose systems, and how to add or change a style — is specced in
