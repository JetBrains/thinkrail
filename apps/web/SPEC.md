...
- **Deployment obligation:** one built client serves every launcher and future deployment. Endpoint selection
  ...
  a deployment name. The shell may progressively enhance native window chrome through a validated optional
  capability injected before React; it branches on capability, never deployment identity, and creates no
  domain/view-state authority.
...
access to the optional native stable string adapter), `nativeWindowChrome.ts` (validation/access for the
optional native presentation capability), and `styles/` — which holds the three
...
- `shell` → children `shell/layout` + `shell/layoutState`, `panels`, `chat` (app-integration render/hydration only), `store`, `transport` (domain hydration + endpoint identity), `contracts` (type-only), `components/ui`, `components` (`ErrorBoundary` around each mounted region + `QuietScrollArea` around shell-owned tool bodies), `constants`, `nativeWindowChrome` (optional native presentation only), `lib` (platform shortcut semantics), `themes` (the single owner of catalog/media resolution and atomic theme application, driven by the hydrated store preference or pre-hydration hint)
...
- leaves (`clientPreferences`, `nativeWindowChrome`, `constants`, `utils`, `styles`) → none internal
