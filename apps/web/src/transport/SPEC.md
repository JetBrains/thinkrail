...
- **Public surface (barrel):** `initTransport`, `getTransport`, `prewarmWorkspaceSkillLoad`, the three
  ...
  `TransportOptions`, `supportsPlanReview`. `supportsPlanReview` is exported because a sibling panel
...
- **Allowed deps:** `contracts` (method maps, `WS_CHANNELS`, `Project` for welcome + `project.updated`, `SessionEventPayload`
  ...
  `SessionDeletedPayload` for `session.deleted`, `PLAN_REVIEW_SUBAGENT_PROTOCOL_VERSION` for the
  `supportsPlanReview` gate, `provider.changed`, the empty addressed
