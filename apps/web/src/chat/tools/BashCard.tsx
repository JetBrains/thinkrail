...
export function BashCard({ args, result, status }: ToolRenderProps) {
	...
	return (
		<div
			...
			className="overflow-hidden rounded-[var(--radius-sm)] border border-border-default bg-container-terminal-bg font-[var(--font-mono)]"
		>
			...
			<pre
				className={`overflow-auto px-sm py-xs text-xs leading-relaxed ${isError ? "text-feedback-error" : "text-text-default"}`}
			>
