...
- **Chat TODO plan** — the chat's `pi-todos` list surfaced **only in the chat** (engine:
  ...
  **status-ordered, group-first** rendering (`planSections`) — group = task: the **in-progress** task
  ...
  is no side-panel Todo tool — the plan lives in the conversation. The "open as markdown" action opens the
  registered `todo-plan` virtual document keyed by session, rendered by the panels' `MarkdownPreview` — no
  file is written to disk. Its layout reference carries resolver kind + session identity rather than inline
  markdown, so another client can hydrate the same shared placement from the host-owned TODO plan.
