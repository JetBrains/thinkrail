...
test("parses the renderer's verbatim output — the real thing, not a lookalike", () => {
	expect(parseReviewPackage(REAL_PACKAGE)).toEqual({ count: 1, files: ["src/a.ts"] });
...
test("parses count and distinct files from a package", () => {
	const s = parseReviewPackage(
		pkg([comment("rc_1", "a.ts"), comment("rc_2", "a.ts"), comment("rc_3", "b.md")]),
	);
	expect(s).toEqual({ count: 3, files: ["a.ts", "b.md"] });
});

test("review-level (anchorless) comments yield no files", () => {
	expect(parseReviewPackage(pkg([comment("rc_9")]))).toEqual({ count: 1, files: [] });
});
