	...
	return (
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="max-w-[36rem]" data-testid="existing-worktree-dialog">
				...
				{loadError ? (
					<div className="flex flex-col items-start gap-8 rounded-[var(--radius-md)] bg-feedback-error-subtle p-12 text-feedback-error tr-text-ui">
						<p>{loadError}</p>
						<Button
							variant="outline"
							size="sm"
				...
				{candidates?.length === 0 ? (
					<div
						className="rounded-[var(--radius-md)] border border-border-default bg-control-bg p-12 text-text-muted tr-text-ui"
						...
						{candidates.map((candidate) => {
							...
							return (
								<button
									...
									className="flex w-full items-start gap-12 rounded-[var(--radius-md)] border border-border-default bg-control-bg p-12 text-left outline-none transition-colors hover:bg-control-bg-hovered focus-visible:ring-2 focus-visible:ring-primary disabled:cursor-not-allowed disabled:opacity-60"
								>
									<div className="mt-2 flex size-28 shrink-0 items-center justify-center rounded-[var(--radius-md)] bg-container-elevated-bg text-text-muted">
										{opening ? (
											<Loader2 className="size-16 animate-spin" />
										) : (
				...
				{openError ? (
					<p
						className="rounded-[var(--radius-md)] bg-feedback-error-subtle px-12 py-8 text-feedback-error tr-text-ui"
