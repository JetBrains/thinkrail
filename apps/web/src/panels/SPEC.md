...
- **Owns:** `ProjectTree` (+ the `NewWorkspaceDialog` its "+" opens **and** the `ConfirmPopover` its per-row
  ...
  **Opening a project** goes through the shared **`useOpenProject`** hook (reused by `ProjectTree` **and**
  ...
  onboarding overlay via `store.openOnboarding("review")`. **`Onboarding`** has moved to its own module —
  see [[submodule-web-onboarding]] (`apps/web/src/onboarding`) — and is no longer owned here. Also
...
a project picker, the prompt hero, and the reused
  ...
  **store-driven two-pane shell** (left section rail + scrollable content pane; mobile collapses the rail to
  ...
  after the last tab closes. Empty-state surfaces still introduce no onboarding state; the deliberate,
  narrow exception is `AppConfig.onboarding`'s two flags (first-run intro; first-worktree banner), owned
  by the `onboarding` module — nothing else may gate on them. `CenterTabs` also renders ephemeral
