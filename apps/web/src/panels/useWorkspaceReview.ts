 ...
 * panel body is unmounted. Called by `WorkspaceWorkbench` (mounted whenever a workspace is active),
 * NOT by `ReviewPanel`. The read re-anchors server-side, so every fs tick also refreshes anchor states.
 */
