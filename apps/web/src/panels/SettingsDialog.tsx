				...
				<div className="flex min-h-0 flex-1 flex-col md:flex-row">
					...
					>
						{SECTIONS.map(({ id, label, icon: Icon }) => {
							...
							return (
								<button
									...
									className={cn(
										"flex shrink-0 items-center gap-8 rounded-[var(--radius-md)] px-12 py-8 text-left tr-text-ui outline-none transition-colors focus-visible:ring-2 focus-visible:ring-primary",
										active
						...
						{SOON.map(({ label, icon: Icon }) => (
							<span
								key={label}
								className="flex shrink-0 cursor-default items-center gap-8 rounded-[var(--radius-md)] px-12 py-8 text-text-disabled tr-text-ui"
							>
								<Icon className="size-16 shrink-0" />
								{label}
