	...
	return (
		<section data-testid="settings-templates" className="flex flex-col gap-lg">
			...
			{loading ? (
				...
				<p data-testid="templates-error" className="text-text-muted text-sm">
		...
		<section className="flex flex-col gap-sm">
			...
			{templates.length === 0 ? (
				scope === "global" ? (
					...
					<p className="text-text-muted text-xs">No templates yet.</p>
				)
			) : (
				<div className="flex flex-col gap-xs">
					{templates.map((t) => (
			...
			>
				<div className="flex min-w-0 flex-1 flex-col">
					...
					{template.description ? (
						<span className="truncate text-text-muted text-xs">{template.description}</span>
					) : null}
				</div>
