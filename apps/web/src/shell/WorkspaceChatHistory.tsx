...
export function WorkspaceChatHistory({
	...
	const showChatAttention = knownChatCount > 1;
	const historyNeedsAttention =
		showChatAttention &&
		closed.some((chat) => sessionNeedsAttention(attentionByWorkspace, workspaceId, chat.sessionId));
		...
		<DropdownMenu>
			<IconTooltip label="View chat history" wrapTrigger>
				...
				>
					...
					{historyNeedsAttention ? (
						<AttentionDot className="absolute top-2 right-2 size-8" />
					) : null}
				</DropdownMenuTrigger>
			</IconTooltip>
			<DropdownMenuContent align="end" className="min-w-[16rem]">
				...
				{closed.map((chat) => {
					const needsAttention =
						showChatAttention &&
						sessionNeedsAttention(attentionByWorkspace, workspaceId, chat.sessionId);
					return (
						<DropdownMenuGroup
							key={chat.sessionId}
