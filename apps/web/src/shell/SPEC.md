...
A chat center tab's body is the session's selected presentation: `ChatResourceBody` renders `ChatView`, or `PlanPane` in session-view mode when the client view state says `work` (`selectSessionView` — see `store/SPEC.md`). Chat and Work are two views of the SAME agent session; the swap changes no domain state, works in any group of any layout (desktop multi-pane or a future mobile single view), and closing the tab remains a view action.
