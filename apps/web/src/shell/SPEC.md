...
A chat center tab's body is the session's selected presentation: `ChatResourceBody` renders `ChatView`, or `PlanPane` when the client view state says `work` (`selectSessionView` — see `store/SPEC.md`). Chat and Work are two views of the SAME agent session; the swap changes no domain state, works in any group of any layout (desktop multi-pane or a future mobile single view), and closing the tab remains a view action. The workbench threads the strip's per-group **content slot** (`renderTabBody`'s second argument) into the body, and the mounted Chat view portals its header controls (TODO strip/usage/Skills) into that slot — the chat surface has exactly one header row, the group tab strip. The Chat | Work choice lives in the session tab ITSELF: `WorkspaceWorkbench` supplies `renderChatTabLabel` (the chat segment — icon + name, view-aware), `renderChatTabActions` (the always-enabled sibling **Work** button, `RiListCheck3`), and `onChatTabPrimaryClick` (clicking the tab body picks Chat) through feature-agnostic Workbench seams; the segments write only `setSessionView`, and the tab's close button remains a view action on the whole session tab.

Resource opens route to that workspace's last-focused surviving center group. Reopening a canonical resource selects its local placement rather than duplicating it. Resource close does not remove the frame group when it becomes empty. Explicit split/add/remove/merge commands own topology; group removal deterministically rehomes resources from every locally retained workspace view before one state commit. Applying a preset follows the same all-views rule. Moving a singleton tool or resizing/folding/showing a region changes the one frame; moving a file/chat/diff/terminal among existing groups changes only the active workspace view. Pointer/resize drafts stay runtime-local and publish one local transition on completion.
