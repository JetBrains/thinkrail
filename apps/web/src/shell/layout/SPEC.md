...
- **Public surface (`index.ts`):** all current-layout types; workbench renderer/controller; pure mutations, projection, and validation helpers; built-in preset definitions plus instantiate/apply/capture operations; attention fallback helpers; and unavailable-reason results. Callers inject resource renderers and may wrap the layout-supplied default tab icon through a presentation callback; the child receives no feature state. Callers commit complete pure results rather than splicing arrays.
