...
Bottom show/toggle uses the same consume-once transition as left/right, including eligible singleton restoration before offering terminal creation and non-bottom focus recovery on hide. Generic programmatic focus suppresses incidental chat-memory recording for every operation except an activating, non-passive open; direct select paths record their chat target before requesting DOM focus. Singleton-tool actions mutate the one frame; resource opens/moves among existing groups mutate only the named workspace view.
