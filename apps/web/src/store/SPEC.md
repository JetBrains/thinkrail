...
- **Owns:** `appStore.ts` — connection/projects/workspaces state + setters. Connection state has two
  ...
  **`analyticsEnabled: boolean`**, and **`subagentsEnabled: boolean`** ride the same `applyConfig` fold
  (host-owned, fieldwise defaulted/validated from the contracts helpers so an older or malformed host
  snapshot cannot poison geometry) — the Line width, Chat, shared Layout catalog, and Privacy read sides.
