...
- **Public surface (barrel):** `useAppStore`; `selectActiveWorkspace`, `selectWorkspaceById` (the
  ...
  scope, whose sides never move by re-pointing the base — derived here, never re-assembled in a panel),
  `selectWorkspaceTick` (the sync-baseline snapshot; + the `isSkillPath` path predicate it shares with
  `noteFsChanged`), `selectWorkspaceSessionIds` (the deduplicated chat tab + history membership used as a
  reconnect-reconciliation baseline);
