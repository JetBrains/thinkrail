...
- **Owns:** `appStore.ts` — connection/projects/workspaces state + setters. Connection state has two
  ...
  **`activateWorkspaceFromRoute(workspace, sessionId?)`**: it applies the same scope ids, advances the
  ...
  local view/layout-attention/terminal/domain-attention maps and chat runtimes + recency drops the dead id,
  ...
  **`auto_retry_start` mirrors pi's live-context surgery**: pi's `_prepareRetry` trims the failed
  ...
  - **Workspace attention** (`attentionByWorkspace`) is the host's cross-workspace signal for chats the
    ...
    **`applySessionAttention`** folds one live push after transport's Pi-event flush. It may stamp the runtime
    revision that contains the causative event only when that runtime is already reconciled to the current
    connection generation; a missing or older-generation runtime requires a fresh transcript reconciliation.
    **`hydrateSessionAttention`** replaces the map from the authoritative all-workspace snapshot and stamps
    candidates with both the current connection generation and one hydration epoch. Transport buffers
    concurrent pushes until snapshot install. Replaying a push for the **same candidate id** merges readiness
    monotonically: it preserves the strictest reconcile requirement and maximum epoch rather than letting
  ...
  **`closedChatsByWorkspace`** (`ClosedChat[]`, per workspace, most-recent-first), and clears pending
  ...
  its history row/runtime + skill baseline + attention row, records a page-lifetime tombstone, removes queued opens for the
...
- **Public surface (barrel):** `useAppStore`; `selectActiveWorkspace`, `selectWorkspaceById` (the
  ...
  **`sessionNeedsAttention` / `workspaceNeedsAttention` / `projectNeedsAttention`** plus the all-known-chat
  ...
  `Toast` (type), web-local frame/workspace-view/layout-attention selectors and atomic actions, resource render-state types
