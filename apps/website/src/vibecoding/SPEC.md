...
- Desktop downloads and CLI commands are one typed install model. The Quick start picker uses compact macOS / Windows / Linux tabs: one compatibility line and restrained download actions remain expanded, while a native **Install via CLI** disclosure owns the commands and Windows shell tabs. macOS exposes Apple Silicon; Windows exposes x64; Linux exposes x64 and ARM64 without browser architecture guessing. The final CTA does not repeat the picker: it detects a supported desktop OS for one compact download action and links unknown/mobile or alternate-platform visitors back to Quick start. Every link uses GitHub's versionless `releases/latest/download` aliases. macOS/Linux/WSL CLI installs use `install.sh`; PowerShell uses `install.ps1`; Command Prompt explicitly launches PowerShell.
