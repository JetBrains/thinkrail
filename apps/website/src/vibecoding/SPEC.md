...
- Desktop downloads and CLI commands are one typed install model shared by both picker instances. Each selected OS leads with its stable public desktop artifact from GitHub's versionless `releases/latest/download` aliases, followed by a visibly secondary command-line alternative. macOS exposes Apple Silicon; Windows exposes x64; Linux exposes x64 and ARM64 without browser architecture guessing. macOS/Linux/WSL CLI installs use `install.sh`; PowerShell uses `install.ps1`; Command Prompt explicitly launches PowerShell.
