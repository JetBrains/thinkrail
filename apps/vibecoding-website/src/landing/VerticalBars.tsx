...
interface VerticalBarsNoiseProps {
  ...
  /** CSS color for the horizontal lines */
  lineColor?: string;
  /** CSS color for the noise bars */
  barColor?: string;
  lineWidth?: number;
  animationSpeed?: number;
  /** Size of one "pixel" square in px */
  pixelSize?: number;
  ...
  const getRippleInfluence = (x: number, y: number, now: number) => {
    ...
    for (const ripple of ripples.current) {
      ...
      if (Math.abs(distance - radius) < 50) {
        total +=
          (1 - age / 2000) * ripple.intensity * (1 - Math.abs(distance - radius) / 50);
      }
    }
  ...
  const animate = useCallback(() => {
    ...
    for (let i = 0; i < numLines; i++) {
      ...
      for (let x = 0; x < w; x += step) {
        ...
        // wide strokes are built from several identical pixels side by side
