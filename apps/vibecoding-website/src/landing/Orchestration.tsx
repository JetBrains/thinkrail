...
const LANES: Lane[] = [
  ...
  },
  {
    name: "agent-03",
    start: 1143,
    activities: ["Inspect current copy", "Draft empty state", "Waiting for input", "Apply user choice"],
    steps: [
      { i: 0, duration: 1238 },
      { i: 1, duration: 1619 },
...
export function Orchestration() {
  ...
  const measure = useCallback(() => {
    ...
    if (outer) {
      ...
      if (w > 0) setScale(Math.min(1, w / DIAGRAM_WIDTH));
    }
    const wrap = wrapRef.current;
    const main = mainRef.current;
    if (!wrap || !main) return;
    const base = wrap.getBoundingClientRect();
    // the diagram may be visually scaled; normalize measurements back to CSS px
  ...
  const laneState = (li: number, ai: number): "done" | "active" | "waiting" | "queued" => {
    const st = lanes[li]!;
    const waitingIndex = LANES[li]!.steps.find((s) => s.waiting)?.i;
  ...
  return (
    <section ref={sectionRef} id="orchestration" className="scroll-mt-16 border-b border-border-muted bg-container-header-bg">
      <div className="mx-auto max-w-[1200px] px-6 py-8 sm:py-24">
        <p className="label-mono">// Multi-agent orchestration layer</p>
        ...
        {isMobile ? (
                ...
                return (
                  <div key={lane.name} className="flex">
                    <div className="relative w-[26px] shrink-0">
                      ...
                      <span
                        aria-hidden
                        className={`absolute left-[10px] top-0 w-px origin-top bg-primary transition-transform duration-300 ease-linear ${last ? "h-[36px]" : "h-full"}`}
                        style={{ transform: `scaleY(${progress[`branch-${li}`] ?? 0})` }}
                      />
                      <span
                        aria-hidden
                      ...
                      <span
                        aria-hidden
                        className="absolute left-[10px] top-[36px] h-px w-[16px] origin-left bg-primary transition-transform duration-300 ease-linear"
                        style={{ transform: `scaleX(${progress[`branch-${li}`] ?? 0})` }}
                      />
                    </div>
          ...
          <div
            ...
            style={{ height: geo ? geo.height * scale : undefined }}
          >
            <div
              ref={wrapRef}
              className="relative w-[1000px] origin-top-left"
              ...
              {geo && (
                ...
                >
                  <g fill="none" strokeLinecap="round" strokeLinejoin="round">
                    {geo.paths.map((p) => (
                      <path key={`base-${p.id}`} d={p.d} stroke="var(--border-muted)" strokeWidth={1} />
                    ))}
                    {geo.paths.map((p) => {
