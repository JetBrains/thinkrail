...
```json
{
  "thinkrailSid": "sub-uuid",
  "parentThinkrailSid": "parent-uuid",
...
|--------|--------|----------|-------------|
| `subsession/create` | `{ parentThinkrailSid, type, context?, name? }` | `{ thinkrailSid, ... }` | Create and optionally start a subsession |
| `subsession/requestSummary` | `{ thinkrailSid }` | `{ ok: true }` | Ask subsession agent to propose return summary |
| `subsession/approveSummary` | `{ thinkrailSid, text }` | `{ ok: true }` | Approve (possibly edited) summary |
| `subsession/dismissSummary` | `{ thinkrailSid }` | `{ ok: true }` | Close without returning anything |
| `subsession/reviseSummary` | `{ thinkrailSid, feedback }` | `{ ok: true }` | Ask agent to rewrite with feedback |
| `subsession/listChildren` | `{ parentThinkrailSid }` | `{ children: [...] }` | List subsessions of a parent |

### New Notifications

| Notification | Payload | Description |
|-------------|---------|-------------|
| `subsession/summaryProposed` | `{ thinkrailSid, summary }` | Agent proposed a return summary |
| `subsession/returned` | `{ parentThinkrailSid, childThinkRailSid, type, summary }` | Summary approved and ready for parent |
