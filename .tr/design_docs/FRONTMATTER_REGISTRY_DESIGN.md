...
2. **Per-project SQLite database** (`~/.tr/indexes/<project-hash>/index.db`) — a **generated cache** rebuilt from frontmatter, stored in the server data directory (completely outside the project repo), used for fast queries and graph traversal. Never hand-edited.
...
└───────────┼────────────────────┼──────────────────┼──────────┘
     ...
     └──────────────────────┬───────────────────────────┘
                            ...
                            ▼
     ┌──────────────────────────────────────────────────┐
     │  ~/.tr/indexes/<hash>/index.db  (SQLite)     │
     │  ┌─────────┐ ┌───────┐ ┌───────────┐ ┌───────┐  │
...
4. **Index lives outside the repo** — The SQLite index is stored in the server data directory (`~/.tr/`), never inside the project. This eliminates `.gitignore` concerns and keeps the repo clean.
...
The index database is stored outside the project repository, in the server data directory alongside `tr.db`. Each project gets its own subdirectory keyed by a truncated SHA-256 hash of the project's absolute path:

```
~/.tr/                          # server data dir (THINKRAIL_DATA_DIR override)
  tr.db                         # server-wide DB (users, tokens, projects)
  indexes/
...
- Reuses the existing `get_data_dir()` infrastructure and `THINKRAIL_DATA_DIR` override
...
One-time migration tool (`thinkrail migrate-registry` or automatic on first startup):
...
4. **Archive** `registry.json` → `.tr/registry.json.bak` (keep for safety)
...
| `backend/app/rpc/server.py` | Watcher emits events to IndexCoordinator; `_rebuild_on_thinkrailhide()` removed |
...
| `.tr/registry.json` | Replaced by frontmatter + index.db (archived as `.bak`) |
