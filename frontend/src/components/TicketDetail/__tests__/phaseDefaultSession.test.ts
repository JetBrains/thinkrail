...
describe("resolvePhaseDefaultSid", () => {
  ...
  it("returns null when the current phase has no skill (idea/done)", () => {
    ...
    const live = new Map([["a", liveSession({ bonsaiSid: "a", skillId: IMPLEMENT })]]);
  ...
  it("returns null when no attached session matches the phase skill", () => {
    ...
    const live = new Map([
      ["a", liveSession({ bonsaiSid: "a", skillId: "ticket-product-design" })],
    ]);
    expect(resolvePhaseDefaultSid(t, live, [], new Map())).toBeNull();
  });
  ...
  it("prefers a worked-on session over an orphan draft", () => {
    ...
    const live = new Map([
      ["draft", liveSession({ bonsaiSid: "draft", skillId: IMPLEMENT, status: "draft", startedAt: 100 })],
      ["worked", liveSession({ bonsaiSid: "worked", skillId: IMPLEMENT, status: "done", startedAt: 1 })],
    ]);
  ...
  it("orders non-draft matches by descending timestamp", () => {
    ...
    const live = new Map([
      ["old", liveSession({ bonsaiSid: "old", skillId: IMPLEMENT, status: "done", startedAt: 10 })],
      ["new", liveSession({ bonsaiSid: "new", skillId: IMPLEMENT, status: "done", startedAt: 20 })],
    ]);
  ...
  it("resolves skill/status from archived sessions when not live", () => {
    const t = makeTicket({ status: "implementing", sessionIds: ["a"] });
    const arch = [archived({ bonsaiSid: "a", skillId: IMPLEMENT, endedAt: 5 })];
  ...
  it("resolves skill/status from summaries when neither live nor archived", () => {
    const t = makeTicket({ status: "implementing", sessionIds: ["a"] });
    const sums = new Map([["a", summary({ bonsaiSid: "a", skillId: IMPLEMENT, status: "done" })]]);
