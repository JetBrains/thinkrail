...
function PlanViewTab({ milestones, verification }: {
...
export function TicketPlanView({ plan, ticketId, onPlanUpdated }: TicketPlanViewProps) {
  const [tab, setTab] = useState<PlanTab>("view");
    ...
    <div className="ticket-right-panel">
      <div className="ticket-right-header">
        ...
        <div className="ticket-plan-tabs">
          {(["view", "steps", "raw"] as const).map((t) => (
            <button
              key={t}
              className={`ticket-plan-tab ${tab === t ? "ticket-plan-tab--active" : ""}`}
              onClick={() => setTab(t)}
            >
              {t === "view" ? "View" : t === "steps" ? "Steps" : "Raw"}
            </button>
          ))}
        </div>
      </div>
      <div className="ticket-right-body">
        {tab === "view" && (
          <PlanViewTab milestones={milestones} verification={verification} />
        )}
        {tab === "steps" && (
          <PlanStepsTab
