...
export default defineConfig({
	...
	webServer: {
		...
		env: {
			...
			// Register a fake update provider so release awareness is drivable without a published release
			// (a source-run host reports no updater at all — see packages/server/src/dev.ts).
			...(centralMode ? {} : { THINKRAIL_E2E_FAKE_UPDATE: "1" }),
